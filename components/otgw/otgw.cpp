...
float OpenthermGateway::parse_float(uint16_t data) {
    return ((data & 0x8000) ? -(0x10000L - data) : data) / 256.0f;
}

int16_t OpenthermGateway::parse_int16(uint16_t data) {
    return *reinterpret_cast<int16_t*>(&data);
}

int8_t OpenthermGateway::parse_int8(uint8_t data) {
...
void OpenthermGateway::set_heating_circuit_1(OpenthermGatewayClimate* clim) {
    ...
    _heating_circuit_1->heating_circuit->set_callback([=](optional<float> state) {
...
void OpenthermGateway::set_heating_circuit_2(OpenthermGatewayClimate* clim) {
    ...
    _heating_circuit_2->heating_circuit->set_callback([=](optional<float> state) {
        set_heating_circuit_setpoint(*_heating_circuit_2, state);
    });
}

void OpenthermGateway::set_override_thermostat(bool value) {
    _override_thermostat = value;
}
