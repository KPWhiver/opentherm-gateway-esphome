...
async def to_code(config):
    ...
    if CONF_ROOM_THERMOSTAT in config:
        var = cg.new_Pvariable(config[CONF_ROOM_THERMOSTAT][CONF_ID], 30)
    ...
    if CONF_HEATING_CIRCUIT_1 in config:
        var = cg.new_Pvariable(config[CONF_HEATING_CIRCUIT_1][CONF_ID], 90)
    ...
    if CONF_HEATING_CIRCUIT_2 in config:
        var = cg.new_Pvariable(config[CONF_HEATING_CIRCUIT_2][CONF_ID], 90)
